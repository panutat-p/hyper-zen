...
enum IconRenderer {
    ...
    static func makeAppIcon(size: CGFloat) -> NSImage {
        ...
        // Standing monkey — same character as the menu bar, straight limbs for the logo.
        let unitScale = size / 18.0
        drawStandingMonkey(
            canvasSize: size,
            unitScale: unitScale,
            fillColor: monkeyFace,
            strokeColor: monkeyFace
        )
    ...
    private static func drawStandingMonkey(
        canvasSize: CGFloat,
        unitScale: CGFloat,
        fillColor: NSColor,
        strokeColor: NSColor
        ...
        let cx = canvasSize / 2
        let bodyCenter = NSPoint(x: cx, y: canvasSize * 0.44)
        ...
        let leftFoot = NSPoint(x: bodyCenter.x - 1.1 * unitScale, y: bodyCenter.y - 4.5 * unitScale)
        let rightFoot = NSPoint(x: bodyCenter.x + 1.1 * unitScale, y: bodyCenter.y - 4.5 * unitScale)

        let leftShoulder = NSPoint(x: bodyCenter.x - 1.7 * unitScale, y: bodyCenter.y + 1.4 * unitScale)
        let rightShoulder = NSPoint(x: bodyCenter.x + 1.7 * unitScale, y: bodyCenter.y + 1.4 * unitScale)
        let leftHand = NSPoint(x: bodyCenter.x - 3.0 * unitScale, y: bodyCenter.y - 2.2 * unitScale)
        let rightHand = NSPoint(x: bodyCenter.x + 3.0 * unitScale, y: bodyCenter.y - 2.2 * unitScale)

        drawLimb(from: leftHip, to: leftFoot, width: limbWidth)
        drawLimb(from: rightHip, to: rightFoot, width: limbWidth)
        drawLimb(from: leftShoulder, to: leftHand, width: limbWidth)
        drawLimb(from: rightShoulder, to: rightHand, width: limbWidth)
    ...
    private static func drawDetailedMonkeyHead(
        ...
        faceColor.setFill()
        for earOffset in [-headRadius * 0.92, headRadius * 0.92] {
